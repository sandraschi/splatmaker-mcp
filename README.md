...
**Risk framing added 2026-07-13 (Sandra):** Postshot's undocumented free-tier reset is also consistent with a specific pattern worth naming — a skilled solo developer (real pedigree: C4D plugin history) picking up agentic coding tools and entering a high-velocity, pre-release-discipline phase (repos spinning up, ideas shipping, output multiplying) where pricing infrastructure just isn't the current focus, not a deliberate community strategy. Not verified — no public roadmap/changelog explains the reset — but it's a materially worse risk profile for unattended infrastructure than "stable tool, might cost money later": CLI flags, output formats, and docs can all shift mid-transition in ways a subprocess wrapper won't gracefully handle. **Conclusion: Postshot is not a `SplatBackend` candidate even though it's currently free and has a CLI.** Its legitimate role in this fleet, if any, is a hand-driven tool for one-off high-quality captures where Sandra wants its GUI polish — never something `splat_generate` calls unattended.
