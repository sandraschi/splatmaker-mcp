...
7. **`operation="from_prompt"`** — an Ollama-driven prompt-to-spec stage, per DeepSeek V4 Flash's 2026-07-13 cross-examination review (`deepseek comments.md`) and Sandra's follow-up worked example ("Falling Water Villa living room" → disambiguation between real Fallingwater and North by Northwest's fictional Vandamm House). Full pipeline sketch in `docs/FROM_PROMPT_DESIGN.md`: disambiguation → grounding search (plex-mcp + web) → scene spec → comfyops-mcp hero image → comfyops-mcp i2v camera-path video → feeds straight into the already-built `from_video` pipeline. ~90% reuse of existing fleet tools; the technical risk (does i2v maintain enough multi-view consistency for COLMAP?) is flagged unverified, not assumed.
